...
export function Result({ value, hypothetical = false }: { value: CheckIn; hypothetical?: boolean }) {
  ...
  return <section className="panel result" aria-labelledby={headingId}>
    <div className="eyebrow"><Check size={16} aria-hidden="true" /> {hypothetical ? 'Hypothetical scenario · never saved' : `Saved observation · ${value.observation_date}`}</div>
    <h2 id={headingId}>{hypothetical ? 'Your scenario, explained' : 'Your check-in, explained'}</h2>
    ...
    <div className="scores">
      ...
      <div><span>Routine-based index</span><strong>{result.status === 'ok' ? `${result.score?.toFixed(1)} / 100` : 'Unavailable'}</strong><small>{result.status === 'ok' ? `${result.category} index band` : hypothetical ? 'No numerical scenario result' : 'Observation saved without a numerical result'}</small></div>
    </div>
    ...
    <details><summary>How the heuristic reached this result</summary>
    ...
    <p className="muted">This index is an authored heuristic, not a medically validated measurement. It does not diagnose a condition or establish causes.</p>
  </section>
}
  ...
  return <>
    <div className="workspace">
      <section className="panel" aria-labelledby="checkin-heading">
        <div className="eyebrow">One observation at a time</div>
        <h2 id="checkin-heading">{initial ? `Edit ${initial.observation_date} · revision ${initial.revision}` : 'Your daily check-in'}</h2>
        <p>Describe your day. Nothing is recorded until you choose Save check-in.</p>
        <form onSubmit={submit} aria-busy={pending}>
          <fieldset disabled={pending}>
            <legend className="sr-only">Daily observation</legend>
            ...
            <div className="strain-field"><NumericSlider name="reported_strain" label="How strained did you feel on this day?" help="Your report is separate from the calculated index. Skipping leaves it missing; we never infer it." max={10} step={1} unit="/ 10" value={values.reported_strain} onChange={v => changed('reported_strain', v)} />
      ...
      <aside className="side-note"><ShieldCheck size={25} aria-hidden="true" /><h2>A record you can understand</h2><p>Your own report and the routine-based index answer different questions. Both remain visible.</p><ol><li>Record a real observation.</li><li>Save it to your private account.</li><li>Inspect the exact rules behind the index.</li></ol><p className="muted">The index uses a small set of transparent rules. It can help you reflect on routines, but it cannot explain everything about your day.</p></aside>
    </div>
