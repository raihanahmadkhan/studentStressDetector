...
export function Product({ user, onExpired, onUser }: SessionProps & { onUser: (user: CurrentUser | null) => void }) {
  ...
  return <>
    <nav className="product-nav" aria-label="Product">{(['Today', 'Timeline', 'Patterns', 'Explore', 'Settings'] as Screen[]).map((name, i) => {
      const Icon = icons[i]
      return <button key={name} aria-current={screen === name ? 'page' : undefined} onClick={() => navigate(name)}><Icon size={18} aria-hidden="true" />{name}</button>
    })}</nav>
...
function Timeline({ user, onExpired, onDirty, onExplore }: SessionProps & { onDirty: (value: boolean) => void; onExplore: (value: CheckIn) => void }) {
  ...
  return <>
    <section className="panel"><div className="eyebrow">Your record</div><h2>Timeline</h2><p>One real observation per calendar day. Each edit keeps a separate, dated revision.</p>
      ...
      {page && <><p className="small">Showing {range.start} through {range.end}</p>{page.items.length === 0 ? <div className="empty"><h3>No check-ins in this period</h3><p>Try another date range, or record your first real observation in Today.</p></div> : <div className="table-scroll"><table><caption>Latest saved revisions · strain and heuristic use separate scales</caption><thead><tr><th>Date</th><th>Reported strain /10</th><th>Index /100</th><th>Revision</th><th>Details</th></tr></thead><tbody>{page.items.map(value => <tr key={value.id}><td>{value.observation_date}{value.retrospective && <small className="block">Retrospective</small>}</td><td>{value.inputs.reported_strain ?? 'Skipped'}</td><td>{value.assessment.score ?? 'Unavailable'}<small className="block">{value.assessment.model_version}</small></td><td>{value.revision}</td><td><button disabled={pending || editing} onClick={() => open(value)} aria-label={`Open ${value.observation_date}`}>Open</button></td></tr>)}</tbody></table></div>}
...
function PatternsScreen({ user, onExpired }: SessionProps) {
  ...
  const [metric, setMetric] = useState<keyof Inputs>('sleep_hours')
  const { run, pending, error } = useRequest(onExpired)
  useEffect(() => { void run(signal => api.patterns(end, signal), setData, true) }, [end, run])
  return <section className="panel"><div className="eyebrow">Descriptive, personal, transparent</div><h2>Personal patterns</h2><p>Compare your recent 7 calendar days with the preceding 21 days. These are descriptions of what you recorded, not predictions or causes.</p>
    ...
    {data && <><p className="notice">Recent: {data.windows.recent.join(' → ')} · Baseline: {data.windows.baseline.join(' → ')}</p><div className="pattern-grid">{data.metrics.map(item => <article key={item.id} className="metric-card"><h3>{titles[item.metric]}</h3><p className="small">{item.recent_count}/7 recent · {item.baseline_count}/21 baseline reports</p>{item.difference === null ? <><strong>Not enough data</strong><p>Needs at least 4 recent and 10 baseline reports for this measure.</p></> : <><strong>{item.difference > 0 ? '+' : ''}{item.difference.toFixed(2)} {item.metric.endsWith('hours') ? 'hours' : 'points'}</strong><p>Median {item.recent_median} recent versus {item.baseline_median} baseline.</p></>}{item.latest_deviation && <p>On {end}: unusually {item.latest_deviation} than your baseline.{item.persistent_deviation && ' Recorded in the same direction on three consecutive calendar days.'}</p>}<details><summary>Calculation & limits</summary><p>{item.rule}</p><p>Baseline MAD: {item.mad?.toFixed(2) ?? 'unavailable'} · deviation threshold: {item.threshold?.toFixed(2) ?? 'unavailable'}.</p><p>{item.limitation}</p></details></article>)}</div>
      ...
      <ReflectionPanel key={`${end}:${data.history_version}`} source={{ kind: 'weekly', end_date: end, expected_history_version: data.history_version }} user={user} onExpired={onExpired} /><label className="compact-field">Chart measure<select value={metric} onChange={e => setMetric(e.target.value as keyof Inputs)}>{Object.entries(titles).map(([key, name]) => <option key={key} value={key}>{name}</option>)}</select></label><PatternChart patterns={data} metric={metric} label={titles[metric]} />
...
function Explore({ user, onExpired, reference }: SessionProps & { reference?: CheckIn }) {
  ...
  const [values, setValues] = useState<Record<string, string>>(() => Object.fromEntries(fields.map(([key]) => [key, reference ? reference.inputs[key] == null ? '' : String(reference.inputs[key]) : ''])))
  ...
  return <><section className="panel"><div className="eyebrow">A sandbox for reflection</div><h2>Explore a what-if</h2><p>Hypothetical inputs never create observations, revisions, or personal-pattern data. The backend calculates both sides using the same current heuristic.</p>
...
function SettingsScreen({ user, onExpired, onUser }: SessionProps & { onUser: (user: CurrentUser | null) => void }) {
  ...
  return <section className="panel"><div className="eyebrow">Your account, your data</div><h2>Settings & data</h2>
