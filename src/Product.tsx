...
function PatternsScreen({ user, onExpired }: SessionProps) {
  ...
  return <section className="panel"><div className="eyebrow">How your stress changes</div><h2>Stress & routine trends</h2><p>Start with your reported strain, then explore workload, deadlines, and recovery. Compare your last 7 calendar days with the preceding 21. These patterns describe your records; they do not establish causes.</p>
    ...
    {data && <><PatternSummary data={data} titles={titles} />
...
function SettingsScreen({ user, onExpired, onUser, onDirty, onLogout, authPending }: SessionProps & { onUser: (user: CurrentUser | null) => void; onDirty: (dirty: boolean) => void; onLogout?: () => void; authPending: boolean }) {
  ...
  return <section className="panel"><div className="eyebrow">Your account</div><h2>Account & settings</h2>
    ...
    <hr /><h3>Download your stress report</h3><p>Get a visual report with stress and reported-strain charts, plus a table of your daily check-ins. It uses the latest saved values for each day.</p><button className="primary" disabled={pending} onClick={() => { setNotice(''); void run(async signal => { const blob = await api.exportData(signal); return new Blob([visualReport(await blob.text())], { type: 'text/html;charset=utf-8' }) }, blob => { download(blob, 'student-stress-report.html'); setNotice('Your report is ready in Downloads. Open it in a browser to view the charts or choose Print → Save as PDF.') }) }}>Download visual report</button><p className="small muted">A self-contained HTML file you can view in your browser and print or save as PDF.</p><details><summary>Complete data backup</summary><p>Download all saved revisions and their original calculations as JSON for your own records.</p><button disabled={pending} onClick={() => { setNotice(''); void run(signal => api.exportData(signal), blob => { download(blob); setNotice('Your data backup is ready in Downloads.') }) }}>Download data export</button></details>
    ...
    {action && <form onSubmit={destroy}><p>{action === 'account' ? 'Your account and data will be permanently deleted. Signing in with Google again will create a new, empty account. A sign-in within the last 15 minutes is required.' : 'Permanently delete all observations and every revision. Protection against delayed requests prevents deleted records from being recreated by a retry.'}</p><label>Type DELETE to confirm<input value={confirmation} onChange={e => setConfirmation(e.target.value)} autoComplete="off" disabled={pending} /></label><div className="actions"><button className="danger" disabled={pending || confirmation !== 'DELETE'}>Confirm delete {action}</button><button type="button" disabled={pending} onClick={() => setAction(null)}>Cancel deletion</button></div></form>}</div>
